library(sparseSVM)

svm_split_sparse <- function(data, response, depth = 1, max_depth = 30, min_samples = 5,
                             global_levels = NULL, debug = FALSE, root_data = NULL,
                             purity_threshold = 0.95, indices = NULL,
                             lambda = lambda, alpha = 0.1) {

#  browser()
  # Track original indices
  if (is.null(indices)) indices <- seq_len(nrow(data))

  # Stop immediately if no data
  if (nrow(data) == 0) {
    return(list(
      is_leaf = TRUE,
      prediction = NA,
      n = 0,
      class_dist = numeric(0),
      indices = indices
    ))
  }

  if (is.null(root_data)) root_data <- data
  if (is.null(global_levels)) global_levels <- sort(unique(data[[response]]))

  #data[[response]] <- factor(data[[response]], levels = global_levels)

  y_table <- table(data[[response]])
  class_dist <- prop.table(y_table)
  major_class_prop <- max(class_dist)

  # Stopping condition
  if (depth > max_depth || nrow(data) < min_samples ||
      length(unique(data[[response]])) == 1 ||
      min(y_table) < 2 || major_class_prop >= purity_threshold) {
    return(list(
      is_leaf = TRUE,
      prediction = if (length(y_table) > 0) names(which.max(y_table)) else NA,
      n = nrow(data),
      class_dist = class_dist,
      indices = indices
    ))
  }

  # Prepare design matrix and response
  x <- as.matrix(data[, setdiff(names(data), response), drop = FALSE])
  y <- data[[response]]

  # Fit sparse SVM
  sparse_model <- sparseSVM(X = x, y = y, lambda = lambda, alpha = alpha)
  #lambda <- sparse_model$lambda[nlambda]

  # Class predictions (hard decision)
  pred_class <- predict(sparse_model, X = x, type = "class")

  left_idx  <- which(pred_class == global_levels[1])
  right_idx <- which(pred_class != global_levels[1])

  # Subset data and track indices
  left_data  <- data[left_idx, , drop = FALSE]
  right_data <- data[right_idx, , drop = FALSE]
  left_indices <- indices[left_idx]
  right_indices <- indices[right_idx]

  # Stop if children too small
  if (nrow(left_data) < min_samples || nrow(right_data) < min_samples ||
      length(unique(left_data[[response]])) < 2 ||
      length(unique(right_data[[response]])) < 2) {
    return(list(
      is_leaf = TRUE,
      prediction = names(which.max(y_table)),
      n = nrow(data),
      class_dist = class_dist,
      indices = indices
    ))
  }

  # Extract slope and intercept for this node
  # coef_vec <- coef(sparse_model)#[, nlambda]
  # w <- coef_vec[setdiff(names(coef_vec), "(Intercept)")]
  # b <- coef_vec["(Intercept)"]

  # if (length(w) >= 2) {
  #   slope <- -w[1] / w[2]
  #   intercept <- -b / w[2]
  # } else {
  #   slope <- NA
  #   intercept <- NA
  # }

  if (debug) {
    cat("\n---- Node at depth", depth, "----\n")
    print(list(
      class_dist = class_dist,
      slope = slope,
      intercept = intercept,
      nonzero_coef = coef_vec[coef_vec != 0]
    ))
  }

  # Recursive calls
  left_child  <- svm_split_sparse(left_data, response, depth + 1,
                                  max_depth, min_samples, global_levels, debug, root_data, purity_threshold,
                                  left_indices, nlambda, alpha)
  right_child <- svm_split_sparse(right_data, response, depth + 1,
                                  max_depth, min_samples, global_levels, debug, root_data, purity_threshold,
                                  right_indices, nlambda, alpha)

  return(list(
    is_leaf = FALSE,
    model = sparse_model,
  #  slope = slope,
   # intercept = intercept,
    lambda = lambda,
    left = left_child,
    right = right_child,
    depth = depth,
    n = nrow(data),
    class_dist = class_dist,
    indices = indices
  ))
}
