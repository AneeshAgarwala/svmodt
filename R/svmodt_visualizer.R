...
get_tree_features <- function(tree) {
  if (tree$is_leaf) return(tree$features)
  unique(c(
    tree$features,
...
plot_node_boundary <- function(data, node_features, svm_model, scaler,
  ...
  pad_factor <- 0.5
  repeat {
    grid_original <- create_decision_grid(data, plot_features, node_features,
  ...
  subtitle <- if (!boundary_visible) {
    ...
    paste0("\u26a0 Boundary outside plotted range (all points \u2192 ",
           majority, " branch)")
  } else {
    NULL
  }
  ...
  p <- p +
    ggplot2::geom_point(
      ...
      ggplot2::aes(x     = .data[[plot_features[1]]],
                   ...
                   color = response),
      size = 2, alpha = 0.85) +
    ggplot2::scale_fill_manual(values = fill_vals, name = "SVM Side",
                               drop = FALSE) +
...
plot_boundary <- function(tree, data,
  ...
  traverse <- function(node, data_subset, depth = 1, path = "Root") {
    if (!is.null(max_depth) && depth > max_depth) return(invisible(NULL))
    if (node$is_leaf || is.null(node$model))      return(invisible(NULL))
    ...
    # Plot this node
    result <- tryCatch(
      plot_node_boundary(data_subset, node_features, node$model, node$scaler,
                         response_col, title, resolution),
      error = function(e) {
        warning("plot_boundary: could not plot depth ", depth,
                " (", path, "): ", e$message)
        NULL
      }
    )
...
#' plot_surface(tree_w, wine, response = "class",
...
plot_surface <- function(tree, data, response,
  ...
  if (is.null(features)) {
    ...
    if (length(root_feats) < 2)
      stop("plot_surface: root node uses fewer than 2 features; ",
  ...
  if (!all(features %in% names(data)))
    stop("plot_surface: features not found in data: ",
...
#' viz <- plot(tree, data = wdbc, response = "diagnosis",
#'             plot.type = "boundary")
#' viz$plots[[2]]  # second node
#'
#' # Global decision surface
#' plot(tree, data = wdbc, response = "diagnosis",
#'      plot.type = "surface")
#'
...
plot.svmodt_node <- function(x, y = NULL, ...,
                             ...
                             resolution     = NULL) {
  if (!is.null(y))
    stop("plot.svmodt_node: unexpected positional argument 'y'.\n",
