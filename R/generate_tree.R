generate_tree <- function(target, features, criteria_type = c("gini", "info_gain", "gain_ratio"),
  ...
  if (is.null(all_levels)) {
    all_levels <- levels(factor(target))  # keep all classes from original dataset
  }
  ...
  # --- pruning redundant branch ---
  if (!is.null(left_subtree$prediction) &&
      !is.null(right_subtree$prediction) &&
      left_subtree$prediction == right_subtree$prediction) {

    # collapse into single leaf
    probs <- class_probabilities(target, all_levels)
    prediction <- names(probs)[which.max(probs)]
    return(list(prediction = prediction, probs = probs))
  }
