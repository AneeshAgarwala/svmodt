...
```
Figure \@ref(fig:penguins-viz) depicts the root node of the SVM-based oblique decision tree, where the first split is performed
...
```
Figure \@ref(fig:penguins-viz-2) shows a node at depth 2 of the tree, where the SVM uses bill_length_mm and
...
```{r wine-multiclass-viz, fig.height=6, fig.width=8}
#| fig-cap: SVMODT Decision Surface on Wine Dataset
plot_surface(tree = tree_wine, data = train_wine, response = "class")
```

Figure \@ref(fig:wine-multiclass-viz) displays multiclass decision surface on the Wine dataset across two features in the tree. 
...
Each internal node shows the class used for the one-vs-rest binary split (`split: X vs rest`), the features selected at that node, and whether feature penalization was active (`penalty = !`).
