...
for (i in 1:200) {
  split_data <- initial_split(iris, prop = 0.8, strata = Species)
  ...
  rpart_mod <- rpart(formula = Species~., data = iris, control =  rpart.control(maxdepth = 3))
  #rpart.plot(rpart_mod)
  pred_rpart <- predict(rpart_mod, x_test, type = "class")
  rpart_vec[i] <- mean(pred_rpart != y_test)
  
  tree1 <- generate_tree(y_train, features = x_train, criteria_type = "gini", max_depth = 4, p_stop = 0.01)
  #print_tree(tree1)
